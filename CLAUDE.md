...
| `ui/hud.gd` | Scorebug HUD: three-column dark panel top-left — teams+scores (away top, home bottom with colored badges) | shots-on-goal (away/SHOTS/home) | period+clock. Phase banner (GOAL!, FACEOFF, etc.) is a separate centered panel below. Small dim `v{BuildInfo.VERSION}` tag bottom-right with a "Report Bug" button above it that `OS.shell_open`s a prefilled GitHub new-issue URL (title + body carry version and `OS.get_name()`). Receives scores/phase/period/clock/SOG via GameManager signals; polls `skater.is_elevated` each frame. |
