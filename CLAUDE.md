...
| `domain/rules/player_rules.gd` | Team balancing, two-color generation (`generate_primary_color` / `generate_secondary_color`), faceoff position lookup. Home team: red primary (slot-varied) + black secondary. Away team: white primary + blue secondary (slot-varied). |
